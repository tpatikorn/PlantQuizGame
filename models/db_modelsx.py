...
class ImageX(AbstractDatabaseObject):
...
class TagX(AbstractDatabaseObject):
...
class UserX(AbstractDatabaseObject):
...
if __name__ == "__main__":
    print(ImageX.get_query(["tag_ids", "active"]))
    print(ImageX.get_query(["active"]))
    print(ImageX.get_query(["tag_ids"]))

    from connectors import db_connector as dbc

    x = dbc.select_all(ImageX.get_query(["tag_ids"]), [(21, 20)])
    print(*[ImageX(_) for _ in x], sep="\n")
